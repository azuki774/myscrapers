...
- メンテナンス時: `status: "maintenance"` を含む部分的な JSON も、FIGI を解決できた場合は通常の成功結果として同様に保存する。
- 失敗時:
...
- `schema_version` は破壊的変更日を表す `YYYY-MM-DD` 形式の文字列。現在のバージョンは **2026-09-12**（実装上の単一の定数 `sbi.CurrentSchemaVersion`）。書き出し時に必ず付与されるため、stdout 出力・ファイル出力のどちらでも常に先頭キーとして現れる。
- **バージョンを上げる（破壊的変更）**: フィールドの削除・リネーム、フィールドの型変更、既存フィールドの値の意味変更など、旧バージョンを前提とした取り込み処理が壊れる変更。その変更をリリースする日付を新しい `YYYY-MM-DD` として設定する。
